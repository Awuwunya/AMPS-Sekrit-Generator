...
.meta
	...
	if FEATURE_MODTL
tlmod	macro name
	bra.w	%macpfx%name%macpfx%_1			%at%; jump for operator 1
	bra.w	%macpfx%name%macpfx%_2			%at%; jump for operator 2
	bra.w	%macpfx%name%macpfx%_3			%at%; jump for operator 3
	bra.w	%macpfx%name%macpfx%_4			%at%; jump for operator 4
    endm
...
copychFM3SM	macro ch, type
	move.w	#($100<<cfbRun)|($100<<cfbVol)|%macpfx%type,%macpfx%ch+cFlags.w; enable channel tracker and set type
	move.l	a4,%macpfx%ch+cData.w			; save data address

	move.b	#cSize,%macpfx%ch+cStack.w		; set stack address
	move.b	#1,%macpfx%ch+cDuration.w		; set duration to expire next frame

	move.b	mFM3op1+cDetune.w,%macpfx%ch+cDetune.w	; copy detune
	move.w	mFM3op1+cPitch.w,%macpfx%ch+cPitch.w	; copy transposition and volume
	move.b	mFM3op1+cVoice.w,%macpfx%ch+cVoice.w	; copy voice (DOES NOT UPDATE IT!!)
	move.b	mFM3op1+cLastDur.w,%macpfx%ch+cLastDur.w; copy last duration
	move.w	mFM3op1+cFreq.w,%macpfx%ch+cFreq.w	; copy frequency
	move.w	mFM3op1+cGateCur.w,%macpfx%ch+cGateCur.w; copy note gate
    endm
...
; ---------------------------------------------------------------------------
	...
	if FEATURE_PSG4
		...
		move.b	mPSG4+cStatPSG4.w,dPSG%laddr%; update PSG4 status to PSG port
...
.cxpsg3
		moveq	#%mvq%ctPSG3,d3%at%	; use PSG3 as primary channel
...
; ---------------------------------------------------------------------------
	...
	if FEATURE_FM3SM
dcCSMOn:
		moveq	#%mq%81,d3%at%		; prepare CSM enable value to d3
